...
# Multiple dispatch methods for combine_add — avoids isa() chain overhead
combine_add(a::ScalarField, b::ScalarField) = a + b
combine_add(a::ScalarField, b::Number) = add_scalar_to_field(a, b)
combine_add(a::Number, b::ScalarField) = add_scalar_to_field(b, a)
combine_add(a::Number, b::Number) = a + b
combine_add(a::VectorField, b::VectorField) = add_vector_fields(a, b)
combine_add(a::VectorField, b::Number) = add_scalar_to_vector_field(a, b)
combine_add(a::Number, b::VectorField) = add_scalar_to_vector_field(b, a)
combine_add(a::VectorField, ::ScalarField) = throw(ArgumentError("Cannot add VectorField and ScalarField directly - incompatible types"))
combine_add(::ScalarField, b::VectorField) = throw(ArgumentError("Cannot add ScalarField and VectorField directly - incompatible types"))
combine_add(a::AbstractArray, b::AbstractArray) = _binary_array_op(+, a, b)
combine_add(a, b) = a + b  # Generic fallback
...
# Multiple dispatch methods for combine_multiply — avoids isa() chain overhead
combine_multiply(a::ScalarField, b::ScalarField) = a * b
combine_multiply(a::ScalarField, b::Number) = a * b
combine_multiply(a::Number, b::ScalarField) = b * a
combine_multiply(a::Number, b::Number) = a * b
...
function cross_operands(a::VectorField, b::VectorField)
    ...
    result[1] = handedness * (a.components[2] * b.components[3] - a.components[3] * b.components[2])
    result[2] = handedness * (a.components[3] * b.components[1] - a.components[1] * b.components[3])
    result[3] = handedness * (a.components[1] * b.components[2] - a.components[2] * b.components[1])
