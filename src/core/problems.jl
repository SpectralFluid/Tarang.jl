...
function parse_linear_expression(expr_str::AbstractString, namespace::Dict{String, Any})
    ...
    if !is_linear_expression(expr, namespace)
        @debug "Non-linear terms detected on LHS of equation. Consider moving to RHS: $expr_str"
        @debug "Requires: Linear terms (LHS) = Nonlinear terms (RHS)"
    end
...
# following the Dedalus pattern (dedalus/core/field.py:has and arithmetic.py:has)

"""
    is_linear_expression(expr, namespace::Dict{String, Any}) -> Bool

Check if expression is linear in the problem variables.
Follows the Dedalus approach (dedalus/core/arithmetic.py):
...
function is_linear_expression(expr, namespace::Dict{String, Any})
    ...
    # Use require_linearity() following Dedalus pattern
...
function validate_equation_structure(LHS, RHS, original_equation::String)
    ...
    if !is_valid_lhs
        @debug "LHS may not follow linear structure: $original_equation"
        @debug "Ensure LHS contains only: dt(vars), linear spatial derivatives, constant coefficients"
        if lhs_info[:error_message] !== nothing
            @debug "Issue: $(lhs_info[:error_message])"
        end
    end
...
# These types never contain problem variables (following Dedalus pattern)
...
function expression_to_string(expr)
    ...
    elseif isa(expr, DivideOperator)
        ...
        return "($left_str / $right_str)"
    elseif isa(expr, PowerOperator)
        base_str = expression_to_string(expr.left)
        exp_str = expression_to_string(expr.right)
