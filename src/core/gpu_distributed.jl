...
function local_fft_dim!(data, dim::Int, dfft::DistributedGPUFFT)
    ...
    if !(arch isa CPU)
        @warn "local_fft_dim! falling back to CPU roundtrip — load CUDA extension for GPU-native FFT" maxlog=1
    end
...
function local_ifft_dim!(data, dim::Int, dfft::DistributedGPUFFT)
    arch = architecture(data)
    if !(arch isa CPU)
        @warn "local_ifft_dim! falling back to CPU roundtrip — load CUDA extension for GPU-native IFFT" maxlog=1
    end
    data_cpu = on_architecture(CPU(), data)
