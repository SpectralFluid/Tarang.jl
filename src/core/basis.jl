...
For all MPI parallelization: only pure Fourier domains are supported.
For TransposableField (use_pencil_arrays=false): only ComplexFourier is supported.
For PencilArrays (use_pencil_arrays=true): both ComplexFourier and RealFourier are supported
because PencilFFTs properly handles RFFT transforms.

Note: RealFourier is not supported for TransposableField because its half-spectrum (rfft)
layout is incompatible with the custom transpose operations. PencilFFTs/PencilArrays
handles RFFT correctly through its Transforms.RFFT() type.
"""
function validate_mpi_fourier_only(bases, nprocs::Int; use_pencil_arrays::Bool=true)
    ...
    # Check for non-Fourier bases (applies to all MPI parallelization)
    if !is_pure_fourier_domain(bases)
        non_fourier = [typeof(b).name.name for b in bases if !is_fourier_basis(b)]
        error("MPI parallelization (nprocs=$nprocs) only supports Fourier domains. " *
              "Found non-Fourier bases: $(join(non_fourier, ", ")). " *
              "Use serial execution (nprocs=1) for Chebyshev, Jacobi, or mixed bases.")
    end

    # Check for RealFourier (only compatible with PencilArrays which supports RFFT)
    # TransposableField's custom transposes don't support RealFourier's half-spectrum layout
    if !use_pencil_arrays && !is_pure_complex_fourier_domain(bases)
        real_fourier = [typeof(b).name.name for b in bases if is_fourier_basis(b) && !is_complex_fourier_basis(b)]
        error("MPI parallelization with TransposableField (nprocs=$nprocs) requires ComplexFourier bases. " *
              "Found RealFourier bases: $(join(real_fourier, ", ")). " *
              "RealFourier uses rfft (half-spectrum) which is incompatible with custom MPI transposes. " *
              "Use ComplexFourier for TransposableField, or enable PencilArrays (use_pencil_arrays=true) " *
              "which properly supports RFFT transforms.")
    end
