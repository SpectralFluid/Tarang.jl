...
mutable struct NetCDFFileHandler
    ...
    # Independent schedule tracking (avoids cross-contamination between criteria)
    last_sim_div::Int   # Last sim_time interval that triggered a write
    last_wall_div::Int  # Last wall_time interval that triggered a write
...
function _append_file_state(filename::String, handler::NetCDFFileHandler)
    ...
    attrs = netcdf_file_info(filename).gatts
    # Unlimited dimensions can include an unfinished tail. Only committed records
    # advance append state; older files without a marker retain legacy behavior.
    record_count = Int(get(attrs, "committed_writes", length(sim_times)))
    0 <= record_count <= length(sim_times) || error(
        "append recovery: '$filename' has an invalid committed record count")
    sim_times = sim_times[1:record_count]
    total_count = Int(get(attrs, "committed_total_writes", record_count))
    if record_count > 0 && group_var_exists(filename, NETCDF_TIME_GROUP, "write_number")
        ...
        total_count = Int(write_numbers[record_count])
    end
    ...
    if handler.wall_dt !== nothing && record_count > 0
        ...
        last_wall_div = floor(Int, wall_times[record_count] / handler.wall_dt)
    end
...
function build_layout_metadata(task::Dict, operator, data)
    ...
    Tuple(size(data)[lead_dims+1:end]) == Tuple(local_shape) || throw(DimensionMismatch(
        "output task '$(task["name"])': staged spatial shape $(size(data)[lead_dims+1:end]) " *
        "does not match slab metadata $(Tuple(local_shape))"))
...
IMPORTANT: For coefficient layout (:c) with RealFourier bases, the first
RealFourier dimension has N/2+1 coefficients (RFFT), not N.
"""
function get_global_shape(layout, domain_info, scales)
    ...
    # CRITICAL FIX: For coefficient layout with RealFourier bases,
    # the first RealFourier dimension uses RFFT with N/2+1 coefficients
    layout_sym = normalize_layout_symbol(layout)
    if layout_sym == :c
        # Grid scales change collocation points, not the retained spectral modes.
        return _apply_rfft_shape_correction(base_shape, domain_info)
    end
...
"""
    _apply_rfft_shape_correction(base_shape, domain_info)

For coefficient layout, correct the shape for RealFourier bases.
The first RealFourier axis uses RFFT with N/2+1 coefficients instead of N.
Subsequent RealFourier axes use full FFT with N coefficients.

This matches PencilFFTs behavior where only the FIRST RealFourier axis
applies the RFFT optimization.

CRITICAL: This function requires domain context (domain_info["domain"]) to correctly
identify the first RealFourier axis. If domain context is missing, no correction
is applied and a warning is logged.
"""
function _apply_rfft_shape_correction(base_shape, domain_info)
    # Check if domain key exists in domain_info
    if !haskey(domain_info, "domain")
        @warn "RFFT shape correction: domain_info missing 'domain' key. " *
              "Coefficient shape may be incorrect for RealFourier bases. " *
              "Returning uncorrected shape: $base_shape" maxlog=1
        return base_shape
    end

    domain = domain_info["domain"]
    if domain === nothing
        @debug "RFFT shape correction: domain is nothing, skipping correction" maxlog=1
        return base_shape
    end

    if !hasfield(typeof(domain), :bases) || domain.bases === nothing
        @debug "RFFT shape correction: domain has no bases, skipping correction" maxlog=1
        return base_shape
    end

    bases = domain.bases
    if length(bases) != length(base_shape)
        @warn "RFFT shape correction: bases length ($(length(bases))) != shape length ($(length(base_shape))). " *
              "Cannot determine RFFT axis. Returning uncorrected shape." maxlog=1
        return base_shape
    end

    # Find the first RealFourier axis (if any)
    first_real_fourier_idx = nothing
    for (i, basis) in enumerate(bases)
        if isa(basis, RealFourier)
            first_real_fourier_idx = i
            break
        end
    end

    if first_real_fourier_idx === nothing
        # No RealFourier bases, shape is unchanged
        return base_shape
    end

    # Apply RFFT correction: N -> N/2+1 for the first RealFourier axis only
    corrected_shape = collect(base_shape)
    N = corrected_shape[first_real_fourier_idx]
    corrected_N = div(N, 2) + 1
    corrected_shape[first_real_fourier_idx] = corrected_N

    # Validate the correction makes sense
    if corrected_N <= 0
        @warn "RFFT shape correction produced non-positive size: N=$N -> corrected=$corrected_N. " *
              "This indicates incorrect domain setup." maxlog=1
        return base_shape
    end
...
function check_schedule(handler::NetCDFFileHandler; iteration=0, wall_time=0.0, sim_time=0.0, timestep=0.0)
    ...
    # Simulation time cadence - uses independent last_sim_div counter
    # to avoid cross-contamination from writes triggered by other criteria
    if handler.sim_dt !== nothing
        has_schedule = true
        sim_div = floor(Int, sim_time / handler.sim_dt)
        if sim_div > handler.last_sim_div
            scheduled = true
        end
    end

    # Wall time cadence - uses independent last_wall_div counter
    if handler.wall_dt !== nothing
        has_schedule = true
        wall_div = floor(Int, wall_time / handler.wall_dt)
        if wall_div > handler.last_wall_div
            scheduled = true
        end
    end
...
function create_current_file!(handler::NetCDFFileHandler; create::Bool=true)
    ...
    global_attrs = Dict(
        ...
        "committed_writes" => 0,
        "committed_total_writes" => max(0, handler.total_write_num - 1),
...
function process!(handler::NetCDFFileHandler; iteration=nothing, wall_time=nothing, sim_time=nothing, timestep=nothing)
    ...
    try
        ...
        _output_collectively(handler, "process! time metadata") do
            # Also establishes the marker before extending legacy append files.
            ncputatt(filename, "global", Dict(
                "committed_writes" => write_index - 1,
                "committed_total_writes" => handler.total_write_num - 1))
        ...
        _output_collectively(handler, "process! commit record") do
            ncputatt(filename, "global", Dict(
                "committed_writes" => write_index,
                "committed_total_writes" => handler.total_write_num))
        end
    catch e
...
function physical_coordinate_data(task::Dict, operator, data_shape::Tuple, data_dim_index::Int, is_complex_data::Bool)
    ...
    scales = task["scales"]
    scale = scales === nothing ? 1.0 : (scales isa Number ? scales : scales[spatial_index])
    native = _native_grid(basis, scale)
    full_coord = collect(basis isa FourierBasis ? native :
                         basis.meta.COV !== nothing ? problem_coord(basis.meta.COV, native) :
...
function output_dimension_name(task_name::String, task::Dict, operator, data_dim_index::Int, is_complex_data::Bool)
    ...
    scales = get(task, "scales", nothing)
    scaled = scales !== nothing && (scales isa Number ? scales != 1 : any(!=(1), scales))
    # NetCDF dimensions are shared by name. Different output resolutions must
    # not reuse the base grid's dimension or coordinate variable.
    coord_name !== nothing && scaled && return "$(task_name)_$(coord_name)"
...
function _stage_task_data!(handler::NetCDFFileHandler, task::Dict,
    ...
    data = nothing
    if isa(operator, ScalarField)
        data = _stage_scalar_field!(stage_cache, operator, layout_symbol; scales=task["scales"])
    elseif isa(operator, VectorField)
    ...
    elseif isa(operator, TensorField)
        ...
        for i in 1:size(comps, 1), j in 1:size(comps, 2)
            ...
            data_arr[indices...] = _stage_scalar_field!(stage_cache, comps[i, j], layout_symbol; scales=task["scales"])
        end
        data = data_arr
    else
...
function _stage_scalar_field!(cache::NetCDFStagingCache, field::ScalarField, layout::Symbol;
    ...
    output_scales = scales === nothing ? field.scales : remedy_scales(field.dist, scales)
    key = (objectid(field), norm_layout, output_scales)
...
Writes final metadata attributes to the current NetCDF file.
Also clears the internal variable-creation cache.

Note: NetCDF.jl's filename-based API (nccreate, ncwrite, ncread, ncputatt)
opens and closes the underlying file descriptor on each call, so there is
no persistent file handle to release here. This method exists to flush
final metadata and reset handler state.
"""
...
"""
    Base.close(handler::NetCDFFileHandler)

Standard `close` interface for NetCDFFileHandler, enabling `close(handler)`
and use as a finalizer. Delegates to `close!`.
"""
Base.close(handler::NetCDFFileHandler) = close!(handler)
...
function reset!(handler::NetCDFFileHandler; keep_tasks::Bool=true)
    # Finalize current file before resetting
    close!(handler)
